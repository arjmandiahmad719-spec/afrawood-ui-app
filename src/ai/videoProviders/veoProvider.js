function makeJobId() {
  return `veo_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
}
...
function getBaseUrl() {
  return import.meta.env.VITE_VEO_BASE_URL || "";
}

export async function createVeoJob(request = {}) {
  ...
  try {
    ...
    return {
      ...
      aspectRatio: request.aspectRatio,
      hasAudio: false,
      error: error?.message || "Veo request failed.",
    };
  }
}

export async function getVeoJobStatus(job) {
  const baseUrl = getBaseUrl();
  const apiKey = import.meta.env.VITE_VEO_API_KEY;
  ...
  try {
    ...
    return {
      ...job,
      status: "failed",
      error: error?.message || "Failed to fetch Veo status.",
    };
  }
}