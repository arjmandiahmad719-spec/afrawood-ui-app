...
export function normalizeProject(project) {
  ...
  const imagesSource = safeObject(source.images);
  const normalizedImages = Object.entries(imagesSource).reduce(
    (acc, [shotId, record]) => {
...
export function exportProjectSnapshot(project, chatHistory = [], uiState = {}) {
  ...
  const fileName = `${safeText(
    ...
    .toLowerCase()
    .replace(/[^\p{L}\p{N}]+/gu, "-")
    .replace(/^-+|-+$/g, "") || "afrawood-project"}-snapshot.json`;
